...
(function($){
	...
	$.ocupload = function(element, options) {
		...
		var iframe = $(
			'<iframe '+
				'id="iframe'+options.iframeName+'" '+
				'name="iframe'+options.iframeName+'"'+
			'></iframe>'
		).css({
			display: 'none'
		});
		...
		var form = $(
			'<form '+
				'id="form_' + options.iframeName + '"' +
				'method="post" '+
				'enctype="'+options.enctype+'" '+
				'action="'+options.action+'" '+
				'target="iframe'+options.iframeName+'"'+
			'></form>'
		).css({
			margin: 0,
			padding: 0
		...
		var input = $(
			'<input '+
				'id="ocupload_param_input_file' + options.iframeName + '" '+
				'name="'+options.name+'" '+
				'type="file" '+
			'/>'
		).css({
		...
		input.change(function() {
			...
			if ($('#ocupload_param_input_file' + options.iframeName).val() == null || $('#ocupload_param_input_file' + options.iframeName).val() == '') {
				return;
			}
		...
		$.extend(this, {
			...
			submit: function() {
				...
				iframe.unbind().load(function() {
					...
					$('#ocupload_param_input_file' + options.iframeName).val('');
